...
-- Task 4: Display the first name, surname, and position of all staff sorting by highest pay first
...
-- Task 5: Add one new record to each table
    ...
    -- Insert data into Product table
        INSERT INTO Product (SupplierID, CategoryID, StatusID, Model, Manufacturer, Type, Price)
        VALUES (1, 1, 1, 'Lawn Shredder X100', 'Mad Garden Tech', 'Electric', 199.99);
    ...
    -- Insert data into Service table
        ...
        VALUES ('Lawnmower Service Agreement 2', 'Just a copy of the normal service agreement the customers must sign.', 25.00);
    ...
    -- Insert data into Staff table
        INSERT INTO Staff (RoleID, StoreID, AddressID, FirstName, LastName, ContactNumber, AnnualLeave, Pay)
        VALUES (1, 1, 1, 'Luis', 'Martins', '+353871234567', 29, 50000);
...
-- Task 6: Delete one record from all tables
    ...
    -- Delete from Product table
        DELETE FROM Product WHERE Manufacturer = 'Mad Garden Tech';
...
-- Task 8: Count how many products are in the stock table (rows only)
    -- My stock table counts stock in different stores, My 'Product' table holds products.
        SELECT COUNT(*) FROM Product; 

-- Task 9: Change the role of all staff with the job title Office worker to Administrator
    SELECT * FROM Staff; -- Check before modification
...
-- Task 12: Update the opening times of the office to say, “closed Sundays”
    ...
    WHERE bh.StoreID = 1;
    -- (Store 1 now is open every day except for Sunday)
...
-- Task 14: Delete all products from the database with a specific manufacturer
    SELECT * FROM Stock WHERE ProductID IN (SELECT ProductID FROM Product WHERE Manufacturer = 'GreenTech'); -- Check before modification
    SELECT * FROM Product WHERE Manufacturer = 'GreenTech'; -- Check before modification
    DELETE FROM Stock WHERE ProductID IN (SELECT ProductID FROM Product WHERE Manufacturer = 'GreenTech'); -- Remove dependency
    DELETE FROM Product WHERE Manufacturer = 'GreenTech'; -- Apply modification to Product table
    SELECT * FROM Product WHERE Manufacturer = 'GreenTech'; 
...
-- HERE, need to change the database,
-- Add 'StartTime' to staff

-- Also need to change need to reaad DurationMonths..
-- Since toure adding tables and shjit.
-- Add a table for manufacturers.



-- Task 17: Show all staff members who have been working there for longer than 4 years
    SELECT * FROM Staff
    WHERE strftime('%Y', 'now') - strftime('%Y', JoinDate) > 4;

-- Task 18: Show all services that the company offers which run for over 3 months and have a minimum of €20 per month
    SELECT * FROM Service
    WHERE DurationMonths > 3 AND MonthlyFee >= 20.00;

-- Task 19: Identify how many customers have the word "road" in their address
    SELECT COUNT(*) FROM Customer
    WHERE UPPER(Address) LIKE '%ROAD%';
...
-- Task 20: Create a view that will show the result of a query drawing information from three tables at once
    ...
    SELECT s.FirstName, s.LastName, r.RoleName, st.StoreName, p.ProductName, o.OrderDate, o.OrderTotalAmount
    FROM Staff s
